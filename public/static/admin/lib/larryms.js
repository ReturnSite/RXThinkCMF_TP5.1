...
layui.define(['jquery','layer','laytpl','element'],function(exports){
    ...
    larryms = function(){
        ...
        this.tit = ['【系统】提示您','【系统】错误提示','【系统】参数配置错误提示','【系统】数据源配置错误','【系统】关闭失败提示','【系统】操作成功','【系统】操作失败','【系统】 Ajax 调试信息控制台'],
        this.config ={
            plusDir: '/common/plus/',
            jqDefined:undefined,
            jqUrl:undefined
    ...
    larryms.prototype.logOut = function(url,ok,no){
       this.confirm('确定退出系统吗！',
           function(){
    	   	   //退出时清除菜单缓存
    	   	   larryms.prototype.cleanCached('larry_menu');
    ...
    //全屏设置
    larryms.prototype.fullScreen = function(obj){
		...
		if (fullscreenElement == null) {
			entryFullScreen();
			$(obj).html('<i class="larry-icon">&#xe8e3;</i>全屏切换');
		} else {
			exitFullScreen();
			$(obj).html('<i class="larry-icon">&#xe8d4;</i>全屏切换');
		}
